...
def add_transaction(amount: float, direction: str, category: str, account: str, description: str, tags: str, date: str = None):
    ensure_csv()
    tid = str(uuid.uuid4())
...
def parse_args():
    ...
    a_add = sub.add_parser("add", help="Add a transaction")
    a_add.add_argument("--amount", type=float, required=True)
    a_add.add_argument("--direction", choices=["income", "expense"], required=True)
